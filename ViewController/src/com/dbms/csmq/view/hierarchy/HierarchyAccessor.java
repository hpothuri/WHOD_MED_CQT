...
public abstract class HierarchyAccessor extends Hierarchy {
    ...
    protected Object resolveMethodExpression(String expression, Class returnType, Class[] argTypes,
                                             Object[] argValues) {
    ...
    public DnDAction processDragAndDropEvent(DropEvent dropEvent, RichTreeTable sourceTree, RichTreeTable targetTree,
                                             TreeModel targetTreeModel, int rootTermScope) {
        ...
        */
        ...
        for (int i = 0; i < keys.length; i++) {
            ...
            if (!targetNode.isEditable()) {
                ...
                String msgTxt =
                    "An " + sourceNode.getQueryLevel() + " cannot be the child of an " + targetNode.getQueryLevel();
                FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, msgTxt, null);
                FacesContext.getCurrentInstance().addMessage(null, msg);
            } else {
    ...
    private Object[][] getChildrenForUpdate(RichTreeTable tree) {
        ...
        while (iterator.hasNext()) {
            ...
            CSMQBean.logger.info(userBean.getCaller() + "  " + VAL_PREDICT_GROUP_ID + ":" +
                                 retVal[rowCount][VAL_PREDICT_GROUP_ID]);
            CSMQBean.logger.info(userBean.getCaller() + "  " + VAL_DICT_CONTENT_ID + ":" +
                                 retVal[rowCount][VAL_DICT_CONTENT_ID]);
    ...
    public Object[][] getChildrenForUpdate() {
        ...
        for (int i = 0; i < changes.length; i++) {
            ...
            while (keys.hasMoreElements()) {
                ...
                CSMQBean.logger.info(userBean.getCaller() + "  " + VAL_TERMCAT + ":" + retVal[rowCount][VAL_TERMCAT]);
                CSMQBean.logger.info(userBean.getCaller() + "  " + VAL_TERMLVL + ":" + retVal[rowCount][VAL_TERMLVL]);
                CSMQBean.logger.info(userBean.getCaller() + "  " + VAL_TERMWEIG + ":" +
                                     retVal[rowCount][VAL_TERMWEIG]);
    ...
    public int processUpdateRelations(RichTreeTable tree, String dictContentID) {
        ...
        */
        ...
        try {
            ...
            msg =
new FacesMessage(FacesMessage.SEVERITY_ERROR, "The following error occurred: " + e.getMessage(), null);
            FacesContext.getCurrentInstance().addMessage(null, msg);
